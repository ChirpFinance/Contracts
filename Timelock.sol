...
contract ChirpTimeLock {
    ...
    function queueUpdateEmissions(uint256 chirpPerBlock) external onlyOwner returns (bytes32 txId) {
        string memory _func = "updateEmissionRate(uint256)";
        uint _timestamp = block.timestamp + TIME_DELAY;
        bytes memory _data = abi.encodeWithSignature(_func, chirpPerBlock);
        txId = getTxId(_func, _data, _timestamp);
        if (queued[txId]) {
            revert AlreadyQueuedError(txId);
        }
